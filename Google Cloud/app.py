...
# Global variables to store status and timers
...
try:
    ...
    print("Continuing without phone detection capability")
    interpreter = None
...
def process_frame(frame):
    ...
    # Convert back to BGR for drawing annotations
    ...
    try:
        ...
        for face in faces:
            ...
            elif current_status == "Driver Drowsy!":
                current_status = "Active"
        ...
        print(f"Error in face detection/landmarks: {e}")
            ...
            try:
                ...
                if success:
                    ...
                    if pitch_angle > PITCH_THRESHOLD:
                        ...
                        head_pose_start_time = None
                        if current_status in ["Head Up!", "Head Down!"]:
                            current_status = "Active"
    ...
    if interpreter is not None:
        try:
            ...
            for i in range(int(num_detections[0])):
                if scores[0][i] > 0.5 and int(classes[0][i]) == 76:  # Phone class
                    phone_detected = True
                    box = boxes[0][i]
                    y_min, x_min = int(box[0] * img_h), int(box[1] * img_w)
                    y_max, x_max = int(box[2] * img_h), int(box[3] * img_w)
                    cv2.rectangle(image, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
                    cv2.putText(image, f"Phone: {(scores[0][i] -0.20):.2f}", 
                              (x_min, y_min - 10),
    ...
    cv2.putText(image, f"Status: {current_status}", (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
def receive_frame():
    try:
        ...
        # Process the frame
        processed_frame, driver_status = process_frame(frame)
